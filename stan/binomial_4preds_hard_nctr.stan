...
parameters {
  ...
  vector<multiplier=sigma_edu>[N_edu - 1] beta_edu_raw;
}
transformed parameters {
  vector[2] beta_sex = [beta_sex_raw, -beta_sex_raw]';

  vector[N_age] beta_age
    = append_row(beta_age_raw, -sum(beta_age_raw));
  vector[N_eth] beta_eth
    = append_row(beta_eth_raw, -sum(beta_eth_raw));
  vector[N_edu] beta_edu
    = append_row(beta_edu_raw, -sum(beta_edu_raw));

  vector[N] eta =  inv_logit(beta_0 + beta_sex[sex] + beta_age[age]
