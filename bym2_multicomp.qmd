...
then the neighbor graph of New York City is has several components and a few islands.

In order to apply the BYM2 model to the NYC dataset, it is necessary to either:

* Edit the neighbor graph to create a fully connected single component for NYC, or
* Extend the BYM2 model to account for disconnected components and islands.

In this notebook, we do the latter.

In [A note on intrinsic Conditional Autoregressive models for disconnected graphs](https://arxiv.org/abs/1705.04854), Freni-Sterrantino et.al. provide the following recommendations:
...
```{python}
bym2_multicompN_mod = CmdStanModel(stan_file=os.path.join('stan', 'bym2_multicompN_ozs.stan'))
```

```{python}
#| scrolled: true
bym2_multicompN_fit = bym2_multicompN_mod.sample(data = nyc_data_dict, output_dir = 'foo')
```

```{python}
bym2_multicomp6_mod = CmdStanModel(stan_file=os.path.join('stan', 'bym2_multicomp6_ozs.stan'))
```

```{python}
bym2_multicomp6_fit = bym2_multicomp6_mod.sample(data = nyc_data_dict, output_dir = 'bar')
```

```{python}
bym2_multicompN_fit.summary().head(20).round(3)
```

```{python}
bym2_multicomp6_fit.summary().head(20).round(3)
```
