...
print("sum_to_zero_vector: runtimes in seconds, all chains")
binomial_ozs_fit.time
```
...
print("sum_to_zero_vector, large: runtimes in seconds, all chains")
binomial_ozs_fit_lg.time
```
...
print("hard sum-to-zero: runtimes in seconds, all chains")	
binomial_hard_fit.time
```

```{python}
binomial_hard_fit_lg = binomial_hard_mod.sample(data=data_large, parallel_chains=4, seed=a_seed)

print("hard sum-to-zero, large: runtimes in seconds, all chains")	
binomial_hard_fit_lg.time
```
...
print("soft sum-to-zero: runtimes in seconds, all chains")	
binomial_soft_fit.time
```

```{python}
binomial_soft_fit_lg = binomial_soft_mod.sample(data=data_large, parallel_chains=4, seed=a_seed)

print("soft sum-to-zero, large: runtimes in seconds, all chains")	
binomial_soft_fit_lg.time
```
...
Although the `ESS_bulk` numbers for the both implmentations are roughly comparable,
the `ESS_sec` for the `sum_to_zero_vector` implementation is over 12 times faster.
