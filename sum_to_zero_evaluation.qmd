...
display(HTML(style_dataframe(si_summary, 3).to_html()))
```
...
what is most important is effective samples **per second**, therefore,
given that the `sum_to_zero_vector` models consistently have the best
wall-clock time for all datasets, they also have the highest ESS/sec.
