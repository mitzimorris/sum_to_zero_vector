...
In the following sections we set up a data-generating program and generate two test datasets,
one large, one small.  Then we run all three implementations of the sum-to-zero constraint
on these datasets and compare the estimates of the group-level regression parameters.
This provides a small demonstration of the behavoir of these three implementations under
different conditions.  A more thorough investigation would include more and different datasets,
test procedures, and visualizations.
...
model efficienty.
(Note:  we're using a development version of CmdStanPy to scrape timing information from the CSV files
...
ozs_fit_timing = binomial_ozs_fit.timing
ozs_total_time = 0
for i in range(len(ozs_fit_timing)):
    ozs_total_time += ozs_fit_timing[i]['total']
...
hard_fit_timing = binomial_hard_fit.timing
hard_total_time = 0
for i in range(len(hard_fit_timing)):
    hard_total_time += hard_fit_timing[i]['total']
...
soft_fit_timing = binomial_soft_fit.timing
soft_total_time = 0
for i in range(len(soft_fit_timing)):
    soft_total_time += soft_fit_timing[i]['total']
...
ozs_fit_timing_lg = binomial_ozs_fit_lg.timing
ozs_total_time_lg = 0
for i in range(len(ozs_fit_timing_lg)):
    ozs_total_time_lg += ozs_fit_timing_lg[i]['total']
...
hard_fit_timing_lg = binomial_hard_fit_lg.timing
hard_total_time_lg = 0
for i in range(len(hard_fit_timing_lg)):
    hard_total_time_lg += hard_fit_timing_lg[i]['total']
...
soft_fit_timing_lg = binomial_soft_fit_lg.timing
soft_total_time_lg = 0
for i in range(len(soft_fit_timing_lg)):
    soft_total_time_lg += soft_fit_timing_lg[i]['total']
...
ozs_fit_timing = brklyn_qns_ozs_fit.timing
ozs_total_time = 0
for i in range(len(ozs_fit_timing)):
    ozs_total_time += ozs_fit_timing[i]['total']
...
hard_fit_timing = brklyn_qns_hard_fit.timing
hard_total_time = 0
for i in range(len(hard_fit_timing)):
    hard_total_time += hard_fit_timing[i]['total']
...
soft_fit_timing = brklyn_qns_soft_fit.timing
soft_total_time = 0
for i in range(len(soft_fit_timing)):
    soft_total_time += soft_fit_timing[i]['total']
...
ozs_fit_timing = bronx_ozs_fit.timing
ozs_total_time = 0
for i in range(len(ozs_fit_timing)):
    ozs_total_time += ozs_fit_timing[i]['total']
...
hard_fit_timing = bronx_hard_fit.timing
hard_total_time = 0
for i in range(len(hard_fit_timing)):
    hard_total_time += hard_fit_timing[i]['total']
...
soft_fit_timing = bronx_soft_fit.timing
soft_total_time = 0
for i in range(len(soft_fit_timing)):
    soft_total_time += soft_fit_timing[i]['total']
...
ozs_fit_timing = bym2_multicomp_ozs_fit.timing
ozs_total_time = 0
for i in range(len(ozs_fit_timing)):
    ozs_total_time += ozs_fit_timing[i]['total']
...
soft_fit_timing = bym2_multicomp_soft_fit.timing
soft_total_time = 0
for i in range(len(soft_fit_timing)):
    soft_total_time += soft_fit_timing[i]['total']
...
For the BYM2_multicomp model, the soft sum-to-zero implementation is painfully slow.
