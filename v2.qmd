...
# from utils_dataviz import * # needed or not?
from utils_nyc_map import nyc_sort_by_comp_size
from utils_bym2 import get_scaling_fact, nbs_to_adjlist
...
We consider the three possible Stan implmentations of this constraint.

The New York City datasets consists of 
counts of events per census tract, child population per census tract,
and a set of collected per-tract features.
...
This model is inadequate for the usual reasons:
the Poisson regression fails to account for the overdispersion present in the data.
Adding an ICAR component helps to account for this.

#### Stan models

The `stan` folder contains implementations `poisson_icar_

The models differ only in their implementation of the sum-to-zero constraint.
...
icar_ozs_fit = icar_ozs_mod.sample(data=nyc_data, output_dir=os.path.join('mcmc_monitor', 'icar_ozs'))
```

```{python}
icar_soft_mod = CmdStanModel(stan_file=os.path.join('stan', 'poisson_icar_soft.stan'))
icar_soft_fit = icar_soft_mod.sample(data=nyc_data, output_dir=os.path.join('mcmc_monitor', 'icar_soft'))
```

```{python}
icar_hard_mod = CmdStanModel(stan_file=os.path.join('stan', 'poisson_icar_hard.stan'))
icar_hard_fit = icar_hard_mod.sample(data=nyc_data, output_dir=os.path.join('mcmc_monitor', 'icar_hard'))
```
...
bym2_ozs_fit = bym2_ozs_mod.sample(data=nyc_data, output_dir=os.path.join('mcmc_monitor', 'bym2_ozs'))
```

```{python}
bym2_soft_mod = CmdStanModel(stan_file=os.path.join('stan', 'bym2_soft.stan'))
bym2_soft_fit = bym2_soft_mod.sample(data=nyc_data, output_dir=os.path.join('mcmc_monitor', 'bym2_soft'))
```

```{python}
bym2_hard_mod = CmdStanModel(stan_file=os.path.join('stan', 'bym2_hard.stan'))
bym2_hard_fit = bym2_hard_mod.sample(data=nyc_data, output_dir=os.path.join('mcmc_monitor', 'bym2_hard'))
```
